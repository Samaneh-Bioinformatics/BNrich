...
author: | 
  | Samaneh Maleknia* (1), Mohsen Namazi (1), Kaveh Kavousi(1), Ali Sharifi-Zarchi (2), Vahid Rezaei Tabar (3)
...
Given that the data used is continuous, each node is modeled as a regression line on its parents (12,15). Thus, on some of these regression lines, the number of these independent variables is high, so in order to avoid the collinearity problem, we need to use the Lasso regression (16,17).
